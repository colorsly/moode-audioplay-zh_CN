...
function getImage($path) {
	...
	switch (strtolower($ext)) {
		...
		case 'tiff':
			...
			break;
		// iso get real filename. by yuyong
		case 'iso':
			$dirpath = pathinfo($path, PATHINFO_DIRNAME) . '/';
			// workerLog('thumb-gen-iso-yuyong: Path: ' . $dirpath);
			$image = parseFolder($dirpath);
			break;
		// Embedded images
		case 'mp3':
...
function parseFolder($path) {
	// Default cover files
	$covers = array(
		'Cover.jpg', 'cover.jpg', 'Cover.jpeg', 'cover.jpeg', 'Cover.png', 'cover.png', 'Cover.tif', 'cover.tif', 'Cover.tiff', 'cover.tiff', 'Front.jpg', 'front.jpg', 
	...
	// suit or sacd iso - up level dir search cover files. by yuyong
	$gpath = dirname($path) . '/';
	foreach ($covers as $file) {
		getImage($gpath . $file);
	}
