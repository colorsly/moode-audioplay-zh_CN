		...
		while ($line) {
			...
			if ($element == 'file') {
				...
				if ($element == 'Genre' || $element == 'Artist' || $element == 'AlbumArtist' || $element == 'Conductor' || $element == 'Performer') {
					...
					// iso get tag. by yuyong
					if ($queue[$idx]['fileext'] == 'iso' && $element == 'Performer' && $value != '') {
						$queue[$idx]['AlbumArtist'] = $queue[$idx]['Artist'];
						$queue[$idx]['Artist'] = $value;
					}
				} else {
					// All other tags
					$queue[$idx][$element] = $value;
