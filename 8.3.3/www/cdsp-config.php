...
// Check
	...
	if($checkResult['valid'] == true) {
		$_SESSION['notify']['title'] = '配置有效';
		$_SESSION['notify']['msg'] = $selectedConfigLabel;
	} else {
		$_SESSION['notify']['title'] = '配置无效';
		$_SESSION['notify']['msg'] = $selectedConfigLabel;
	}
}
// Import (Upload)
...
// Remove
	...
	if ($_SESSION['camilladsp'] != $selectedConfig) { // Can't remove active config
		...
		$_SESSION['notify']['title'] = '配置已删除';
		$_SESSION['notify']['msg'] = $selectedConfig;
		$selectedConfig = null;
	} else {
		$_SESSION['notify']['title'] = '无法删除活动配置';
	}
}
// New pipeline
...
if ($_SESSION['invert_polarity'] != '0' ||
 	$_SESSION['crossfeed'] != '关' ||
 	$_SESSION['eqfa12p'] != '关' ||
 	$_SESSION['alsaequal'] != '关' ||
	$_SESSION['audioout'] == 'Bluetooth' ||
	$_SESSION['multiroom_tx'] == '开' ||
	$_SESSION['multiroom_rx'] == '开')
{
