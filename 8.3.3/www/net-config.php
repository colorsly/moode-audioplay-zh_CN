...
if ($cfgSsid === true) {
	$_saved_networks = '<p style="text-align:center;">没有保存的网络</p>';
} else {
	for ($i=0; $i < count($cfgSsid); $i++) {
		...
		$_saved_networks .= '<input name="cfg-ssid-' . $cfgSsid[$i]['id'] . '" class="checkbox-ctl saved-ssid-modal-delete" type="checkbox"><em>删除</em>';
		$_saved_networks .= '</div>';
		$_saved_networks .= '</div>';
	}
}
...
$_eth0currentip = empty($ipAddr[0]) ? '未使用' : $ipAddr[0];
...
if ($_SESSION['apactivated'] == true) {
	...
	$_wlan0currentip = empty($ipAddr[0]) ? '未使用' :
	'地址: ' . $ipAddr[0] . '<br>' .
	'网络: ' . $ssid[0] . '<br>' .
	'质量: ' . $quality . '% level ' . $level;
}
...
if (empty($_wlan0apdpwd)) {
	phpSession('open');
	$_SESSION['notify']['title'] = '注意';
	$_SESSION['notify']['msg'] = '需要输入接入点密码';
