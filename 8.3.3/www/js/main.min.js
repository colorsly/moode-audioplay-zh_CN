	...
	switch (allGenres.sort((function(e, r) {
		...
		.map((function(e) {
			...
			return {
				...
				imgurl: 0 != r ? "/imagesw/thmcache/" + encodeURIComponent(r) + ".jpg" : "/images/notfound.jpg",
...
var renderGenres = function() {
	...
	renderSongs = function(e) {
		...
		if (document.getElementById("songsList")
			...
			.html(filteredSongs[0].album), g = e && !UI.libPos[0] ? filteredAlbums[UI.libPos[0]].album_artist : filteredSongs[0].album_artist, "Nothing found" == filteredSongs[0].album ? ($("#lib-artistname, #artistsList, #lib-albumyear, #lib-numtracks, #lib-encoded-at")
			...
			if (LIB.filters.artists.length > 0) {
				...
				if ("full_lib" != SESSION.json.library_flatlist_filter) var y = '<div id="lib-flatlist-filter"><i class="far fa-filter"></i> Filtered</div>';
				else y = "";
				$("#lib-coverart-img")
...
function showHideTracks(e) {
	!0 === e || "none" == $("#bottom-row")
		.css("display") ? ($("#tracklist-toggle")
			.html('<i class="fal fa-list sx"></i> Hide tracks'), $("#bottom-row")
			.css("display", "flex"), $("#lib-albumcover")
			.css("height", "calc(50% - env(safe-area-inset-top) - 2.75rem)"), $("#index-albumcovers")
			.attr("style", "display:none!important"), customScroll("tracks", 0, 200)) : ($("#tracklist-toggle")
			.html('<i class="fal fa-list sx"></i> Show tracks'), $("#bottom-row")
...
$("#genreheader, #library-header")
	.on("click", (function(e) {
		"full_lib" == SESSION.json.library_flatlist_filter || 0 != $(e.target)
			...
			.length || "all" != GLOBAL.musicScope ? ("tag" == currentView || "album" == currentView ? (LIB.filters.genres.length = 0, LIB.filters.artists.length = 0, LIB.filters.albums.length = 0, LIB.artistClicked = !1, LIB.albumClicked = !1, $("#tracklist-toggle")
				.html('<i class="fal fa-list sx"></i> Show tracks'), "" != $("#lib-album-filter")
				.val() ? $("#searchResetLib")
				.show() : ($("#searchResetLib")
	...
	.click((function(e) {
		...
		if ($("#radioview-sort-tag span")
			...
			if ("Not installed" == SESSION.json.recorder_status) var s = '<li class="modal-dropdown-text"><a href="#notarget" data-cmd="recorder-status-sel"><span class="text">Not installed</span></a></li><li class="modal-dropdown-text"><a href="#notarget" data-cmd="recorder-status-sel"><span class="text">Install recorder</span></a></li>';
			else s = '<li class="modal-dropdown-text"><a href="#notarget" data-cmd="recorder-status-sel"><span class="text">开</span></a></li><li class="modal-dropdown-text"><a href="#notarget" data-cmd="recorder-status-sel"><span class="text">关</span></a></li><li class="modal-dropdown-text"><a href="#notarget" data-cmd="recorder-status-sel"><span class="text">Uninstall recorder</span></a></li>';
			$("#recorder-status-list")
				.html(s), $("#recorder-status span")
				.text(SESSION.json.recorder_status), $.getJSON("command/recorder-cmd.php?cmd=recorder_storage_paths", (function(e) {
					$("#recorder-storage-list")
						.html(e), $("#recorder-storage span")
						.text(SESSION.json.recorder_storage)
				})), $.getJSON("command/recorder-cmd.php?cmd=recorder_album_tag_list", (function(e) {
				...
				})), $.getJSON("command/recorder-cmd.php?cmd=recorder_untagged_file_list", (function(e) {
					var t = "" == e.count ? "(0)" : "(" + e.count + ")";
					$("#untagged-file-count")
						.text(t);
					var r = '<li class="modal-dropdown-text"><a href="#notarget" data-cmd="delete-recordings-sel"><span class="text">否</span></a></li>';
					for (r += '<li class="modal-dropdown-text"><a href="#notarget" data-cmd="delete-recordings-sel"><span class="text">' + ("0" == t ? "" : "All") + "</span></a></li>", i = 0; i < e.files.length; i++) r += '<li class="modal-dropdown-text"><a href="#notarget" data-cmd="delete-recordings-sel"><span class="text">' + e.files[i] + "</span></a></li>";
					$("#delete-recordings span")
						.text("No"), document.getElementById("recorder-untagged-files")
						.innerHTML = r
				})), $("#radio-manager-modal")
				.modal()
	...
	.click((function(e) {
		...
		$.post("command/cfg-table.php?cmd=upd_cfg_system", {
			...
			if ("Install recorder" == t) notify("installing_plugin", "", "infinite"), $.ajax({
				type: "GET",
				...
				success: function(e) {
					"recorder_installed" == e ? ($("#stream-recorder-options, #context-menu-stream-recorder")
						...
						}), notify(e, "Reboot required", "10_seconds")) : notify(e)
				},
				error: function() {
			...
			else if ("Uninstall recorder" == t) $.post("command/recorder-cmd.php?cmd=recorder_uninstall"), $("#stream-recorder-options, #context-menu-stream-recorder")
				.hide(), notify("recorder_uninstalled", "", "5_seconds");
			else if (!0 === i) $.post("command/recorder-cmd.php?cmd=recorder_storage_change"), $(".playback-context-menu i")
				.removeClass("recorder-on"), $("#menu-check-recorder")
				.css("display", "none"), notify("settings_updated");
			else if (!r || "On" != t && "Off" != t)
				if ("Yes" == $("#tag-recordings span")
					.text()) notify("recorder_tagging", "等待完成消息出现", "infinite"), $("#tag-recordings span")
					.text("No"), $.post("command/recorder-cmd.php?cmd=recorder_tag_files");
				else if ("No" != $("#delete-recordings span")
				.text()) {
				var e = $("#delete-recordings span")
					.text();
				$.post("command/recorder-cmd.php?cmd=recorder_delete_files", {
						file_name: e
					}, (function() {
						notify("recorder_deleted", "正在更新库...", "5_seconds")
					})), $("#delete-recordings span")
					.text("No")
			} else notify("settings_updated"), setTimeout((function() {
				$("#btn-ra-refresh")
					.click()
			}), DEFAULT_TIMEOUT);
			else $.post("command/recorder-cmd.php?cmd=recorder_on_off"), "On" == t ? ($(".playback-context-menu i")
	...
	.on("click", ".lib-track", (function(e) {
		UI.dbEntry[0] = $("#songsList .lib-track")
			...
			.addClass("active"), "Track" == SESSION.json.library_track_play ? ($("#play-track")
				.html(' <i class="fal fa-play sx"></i>播放'), $("#clear-play-track")
				.html(' <i class="fal fa-chevron-square-right sx"></i>清除/播放')) : ($("#play-track")
				.html(' <i class="fal fa-play sx"></i>播放+'), $("#clear-play-track")
				.html(' <i class="fal fa-chevron-square-right sx"></i>清除/播放+'))
	})), $("#playlist-items")
	...
	.click((function(e) {
		switch ($(this)
			...
			case "consume":
				$("#menu-check-consume")
					.toggle();
				var t = $("#menu-check-consume")
					.is(":visible") ? "1" : "0";
				sendMpdCmd("consume " + t);
				break;
			case "repeat":
				$("#menu-check-repeat")
					.toggle();
...
jQuery(document)
	.ready((function(e) {
		"use strict";
		null != window.ui_notify && ui_notify(), GLOBAL.scriptSection = "panels", e("#config-back, #config-home")
			.hide(), e("#config-tabs")
		...
		GLOBAL.sbw = t.width() - t[0].clientWidth, e("body")
			...
			.addClass("custom-scrollbars"), e.getJSON("command/cfg-table.php?cmd=get_cfg_tables", (function(t) {
				...
				if ("y" == s[0] && e("#playback-firstuse-help")
					...
					.css("display", "none") : (e("#ss-style")
						...
						.css("background", "rgba(0,0,0,1)")), "Never" != SESSION.json.scnsaver_timeout && e.post("command/playback.php?cmd=reset_screen_saver"), "Clock Radio" == SESSION.json.clkradio_mode || "Sleep Timer" == SESSION.json.clkradio_mode ? (e("#clockradio-icon")
				...
				e("#artistheader > div")
					.html(i), "Year" == SESSION.json.library_albumview_sort && (e("#index-albums")
						.hide(), e("#index-albumcovers")
						.attr("style", "display:none!important")), SESSION.json.feat_bitmask & FEAT_RECORDER && (e("#radio-manager-stream-recorder")
					...
					.removeClass("active"), -1 != currentView.indexOf("playback") ? (e("#playback-panel")
						...
						.addClass("nogenre")) : "album" == currentView ? makeActive(".album-view-btn", "#library-panel", currentView) : "playlist" == currentView && makeActive(".playlist-view-btn", "#playlist-panel", currentView), GLOBAL.chromium && "1" == SESSION.json.localui && "-on" == SESSION.json.auto_coverview && setTimeout((function() {
			...
			.click((function(t) {
				if ("1" == MPD.json.random && "0" == SESSION.json.ashuffle) sendMpdCmd("next");
				else {
			...
			.click((function(e) {
				"1" == SESSION.json.volmute && volMuteSwitch();
				var t = parseInt(SESSION.json.volknob);
				return setVolume(t > 0 ? t - 1 : 0, "volume_down"), !1
			})), e(".btn-toggle")
			.click((function(t) {
				var a = e(this)
					.data("cmd"),
			...
			.click((function(t) {
				if ("" == e("#new-station-name")
					...
					.val()
					.trim()) notify("blank_entries", "未创建电台", "10_seconds");
				else {
					var a = {
			...
			.click((function(t) {
				if ("" == e("#edit-station-name")
					...
					.val()
					.trim()) notify("blank_entries", "电台未更新", "10_seconds");
				else {
					var a = {
			...
			.click((function(t) {
				if ("" == e("#new-playlist-name")
					.val()
					.trim()) notify("blank_entries", "未创建播放列表");
				else {
					var a = {
			...
			.click((function(t) {
				if ("" == e("#edit-playlist-name")
					.val()
					.trim()) notify("blank_entries", "播放列表未更新");
				else {
					var a = [];
					e("#playlist-items li")
						.each((function() {
			...
			.on("click", ".disconnect-renderer", (function(t) {
				notify("renderer_disconnect", "", "3_seconds"), e.post("command/renderer.php?cmd=disconnect_renderer", {
					job: e(this)
						.data("job")
				})
			})), e(document)
			.on("click", ".turnoff-renderer", (function(t) {
				notify("renderer_turnoff", "", "3_seconds"), e.post("command/renderer.php?cmd=disconnect_renderer", {
					job: e(this)
						.data("job")
				})
			...
			.click((function(t) {
				"panel-footer" != e(this)
					.attr("id") && (coverView && (e("body")
						.removeClass("cv"), e("body")
						.removeClass("cvwide"), "Yes" == SESSION.json.show_cvpb && e("body")
			...
			.on("click", ".multiroom-modal-vol", (function(t) {
				...
				e.post("command/multiroom.php?cmd=set_rx_status", {
					...
					.html("<div class='busy-spinner-btn-rx'>" + GLOBAL.busySpinnerSVG + "</div>"), setTimeout((function() {
			...
			.on("click", "#menu-check-cdsp, #menu-check-consume, #menu-check-repeat, #menu-check-single, #menu-check-recorder", (function(e) {
