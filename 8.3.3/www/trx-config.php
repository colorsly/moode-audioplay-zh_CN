...
if (isset($_POST['multiroom_tx_restart'])) {
	submitJob('multiroom_tx_restart', '', '发送器重新启动', '');
}
...
if (isset($_POST['update_multiroom_tx_bfr'])) {
	if (isset($_POST['multiroom_tx_bfr']) && $_POST['multiroom_tx_bfr'] != $cfgMultiroom['tx_bfr']) {
		...
		$msg = $_SESSION['multiroom_tx'] == 'On' ? '发送器重新启动' : '';
...
if (!isset($_SESSION['rx_hostnames'])) {
	$_rx_hostnames = '发现尚未运行';
} else {
	$_rx_hostnames = $_SESSION['rx_hostnames'] == '找不到接收器' ? $_SESSION['rx_hostnames'] : 'Found: ' . $_SESSION['rx_hostnames'];
}
...
// Receiver
$_select['multiroom_rx'] .= "<option value=\"Disabled\" " . (($_SESSION['multiroom_rx'] == 'Disabled') ? "selected" : "") . ">禁止</option>\n";
